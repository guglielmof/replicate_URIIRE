...
%%
function [] = noempty_split_corpus(splitID, trackID)
    ...
    for smpl = 1:EXPERIMENT.split.sample
        ...
        while ~found
            ...
            for shr = 1:length(EXPERIMENT.split.(splitID).ratio)
                for t=1:height(corpusPool)
                    
                    relevance = corpusPool{t, :}{1}{:, 2};
                    rel_idx = (relevance=='Relevant');
                    rel_docs =  corpusPool{t, :}{1}{rel_idx, 1};
                    tName = corpusPool.Properties.RowNames{t};
                    if ~any(ismember(rel_docs, tmps{shr}))
                        found = false;
                        break
                    end
                end
