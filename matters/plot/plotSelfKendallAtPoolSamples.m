...
% original pool and the measure on the sampled pool across the different
% iterations.
%
%% Synopsis
%
...
% See return parameter |tauBStats| in  <../analysis/computeSelfKendallAtPoolSamples.html
% computeSelfKendallAtPoolSamples> for more information.
...
%% Example of use
%  
%   plotSelfKendallAtPoolSamples('Title', 'Campaign 2014, Ad Hoc', ...
%               'Observed', {p10_selfTauBStats, p100_selfTauBStats, Rprec_selfTauBStats}, ...
%               'Reference', {ap_selfTauBStats, bpref_selfTauBStats, rbp_selfTauBStats}, ....
%               'PlotCI', true, ...
%               'OutputPath', '/output')
%
...
function [] = plotSelfKendallAtPoolSamples(varargin)
    ...
    hold on
        ...
        if ~isempty(outputPath)
            ...
            print(h, '-dpdf', sprintf('%1$sselfTauB_AtPoolSamples_%2$s%3$s.pdf', outputPath, fileNameSuffix, uuid()));
            close(h);
        end;
