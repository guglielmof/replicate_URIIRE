...
%% Synopsis
%
%   [measuredRunSet, poolStats, runSetStats, inputParams] = averagePrecision(pool, runSet, Name, Value)
%  
...
% name are, respectively, |condensedAveragePrecision| and |conAP|.
...
%%
function [measuredRunSet, poolStats, runSetStats, inputParams] = averagePrecision(pool, runSet, varargin)
    ...
    if verbose
        ...
        fprintf('Computing average precision for run set %s with respect to pool %s: %d run(s) and %d topic(s) to be processed.\n\n', ...
    ...
    measuredRunSet.Properties.UserData.name = 'averagePrecision';
    measuredRunSet.Properties.UserData.shortName = 'AP';
    
    if strcmpi(notAssessed, 'condensed')
      measuredRunSet.Properties.UserData.name = 'condensedAveragePrecision';
      measuredRunSet.Properties.UserData.shortName = 'condAP';
    end;
    ...
    if verbose
        fprintf('Computation of average precision completed.\n');
    end;
    ...
    function [varargout] = processTopic(topic)
        ...
        function [measure] = processRun(runTopic)
            ...
            p = cumsum(runTopic{relPos, 'Assessment'}).' ...
                ./ ...
                find(relPos).';
            
            % average over the total number of relevant documents
            measure = sum(p) ./ poolStats{ct, 'BinaryRelevant'};
