%% extendedGradedAveragePrecision
% 
% Computes extended graded average precision (xGAP).

%% Synopsis
%
%   [measuredRunSet, poolStats, runSetStats, inputParams] = extendedGradedAveragePrecision(pool, runSet, Name, Value)
%  
% Note that graded extended graded average precision will be NaN when there are no relevant
...
% |extendedGradedAveragePrecision|; _|shortName|_ is a short name of the computed 
% measure, i.e. |xGAP|; _|pool|_ is the identifier of the pool with respect 
...
% name are, respectively, |condensedExtendedGradedAveragePrecision| and |conXGAP|.
...
%% Example of use
%  
%   measuredRunSet = extendedGradedAveragePrecision(pool, runSet);
%
% It computes the average precision values for a run set. Suppose the run 
...
% Please refer to:
%
% * Ferrante, M., Ferro, N., and Maistro, M. (2014). Rethinking How to Extend Average Precision to Graded Relevance.
%   In Kanoulas, E. Lupu, M. Clough, P. D., Sanderson, Hall, M., Hanbury,
%   A., and Toms, E. G. editors,
%   _Proc. of the 5th International Conference of the CLEF Initiative (CLEF
%   2014)_,pages 19-30. Lecture Notes in Computer Science 8685, Springer 2014 
% 
...
%%
function [measuredRunSet, poolStats, runSetStats, inputParams] = extendedGradedAveragePrecision(pool, runSet, varargin)
    ...
    if verbose
        ...
        fprintf('Computing expected graded average precision for run set %s with respect to pool %s: %d run(s) and %d topic(s) to be processed.\n\n', ...
    ...
    measuredRunSet.Properties.UserData.name = 'extendedGradedAveragePrecision';
    measuredRunSet.Properties.UserData.shortName = 'xGAP';
    
    if strcmpi(notAssessed, 'condensed')
      measuredRunSet.Properties.UserData.name = 'condensedExtendedGradedAveragePrecision';
      measuredRunSet.Properties.UserData.shortName = 'condXGAP';
    end;
    ...
    if verbose
        fprintf('Computation of extended graded average precision completed.\n');
    end;
    ...
    function [varargout] = processTopic(topic)
        ...
        function [measure] = processRun(runTopic)
            ...
            % avoid the case in which we know result is already zero
            % because relevanceDegreeRetrieved and
            % relevanceThresholdProbability are complementary
            %if sum( relevanceDegreeRetrieved (relevanceThresholdProbability ~= 0 ) ) == 0
            %     measure = {0};
            ...
            % La formula da problemi solo nel caso J >= K
            % Ho spostato il calcolo di J prima della ridefinizione dei
            % vettori R e g, in questo modo non quando calcolo la 
            % variabile measure lo faccio solo sulle posizioni dei
            % documenti con rilevanza maggiore di J che nella run non
            % sono presenti quindi resta measure = 0.
            % exclude the initial g_i = 0
            J = find(relevanceThresholdProbability, 1, 'first') - 1;
            
           
            % exclude the last R_i = R_{i+1} = ... = 0 elements
            K = find(relevanceCount, 1, 'last');
            
            % Se aggiungo questa condizione evito di calcolare la matrice
            % delta, in ogni caso il codice funzionerebbe comunque
            if (J >= K)
                measure = {0};
            ...
            relevanceCount = relevanceCount(1:K);
            
            
            % the relevance threshold probabilities, excluding last zero
            % elements
            g = relevanceThresholdProbability(1:K);            
            cg = cumsum(g);
            ...
            for k = 1:runSetStats{ct, cr}.relevantRetrieved
                delta(relPos(1:k), relPos(k)) = cg(min(run(relPos(1:k)), run(relPos(k))));
            end;
            
                            
            A = cumsum(g ./ (fliplr(cumsum(fliplr(relevanceCount)))));
            
            s = find(run > J);
                                 
            % average over the total number of relevant documents
            measure = ( A(run(s)) ./ cg(run(s)) ./ s) ...
                        * ...
                        sum(delta(:, s)).';
