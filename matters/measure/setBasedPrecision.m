%% setBasedPrecision
% 
% Computes the set-based precision.

%% Synopsis
%
%   [measuredRunSet, poolStats, runSetStats, inputParams] = setBasedPrecision(pool, runSet, Name, Value)
%  
% Note that recall is calculated for binary relevance, thus 
% multigraded relevance degrees are mapped to binary relevance.
...
% representing the recall. The |UserData| property of  the table 
...
%%
function [measuredRunSet, poolStats, runSetStats, inputParams] = setBasedPrecision(pool, runSet, varargin)
    
    % check that we have the correct number of input arguments. 
    narginchk(2, inf);
    
    % parse the variable inputs
    pnames = {'ShortNameSuffix', 'MapToBinaryRelevance' 'NotAssessed' 'FixNumberRetrievedDocuments' 'Verbose'};
    dflts =  {[]                 'lenient'              'NotRelevant'         []                          false};
    
    if verLessThan('matlab', '9.2.0')
        [shortNameSuffix, mapToBinaryRelevance, notAssessed, fixNumberRetrievedDocuments, verbose, supplied] ...
    ...
    if verbose
        ...
        fprintf('Computing setBasedPrecision for run set %s with respect to pool %s: %d run(s) and %d topic(s) to be processed.\n\n', ...
    ...
    measuredRunSet.Properties.UserData.name = 'setBasedPrecision';
    measuredRunSet.Properties.UserData.shortName = 'P';
    
    if strcmpi(notAssessed, 'condensed')
      measuredRunSet.Properties.UserData.name = 'condensedSetBasedPrecision';
      measuredRunSet.Properties.UserData.shortName = 'condP';
    end;
    ...
    if verbose
        fprintf('Computation of setBasedPrecision completed.\n');
    end;
    ...
    function [varargout] = processTopic(topic)
        ...
        function [measure] = processRun(runTopic)
            ...
            % calculate precision
            measure = runSetStats{ct, cr}.relevantRetrieved / runSetStats{ct, cr}.retrieved;
