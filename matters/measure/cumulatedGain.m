%% cumulatedGain
% 
% Computes the cumulated gain at each rank position, or at 
% document cut-offs values, or at the recall base, or
% at each relevant retrieved document.

%% Synopsis
%
%   [measuredRunSet, poolStats, runSetStats, inputParams] = cumulatedGain(pool, runSet, Name, Value)
%
% Note that cumulated gain will be NaN when there are no relevant
...
% has to be performed or not, i.e whether CG or nCG has to be computed.
% Default is |false|.
% * *|CutOffs|* (optional) - specifies whether CG at document
% cut-offs values has to be returned instead of CG at each rank
...
% values; _|RelevantRetrieved|_ to compute CG at each relevant
% retrieved document; or, _|LastRelevantRetrieved|_ to return the value of
% the measure at the last relevant retrieved document.
% If not specified, CG at each rank position will be computed.
% * *|RecallBase|* (optional) - a boolean specifying whether CG 
...
%% Example of use
%  
%   measuredRunSet = cumulatedGain(pool, runSet, 'CutOffs', 'Standard');
%
...
function [measuredRunSet, poolStats, runSetStats, inputParams] = cumulatedGain(pool, runSet, varargin)
    ...
    pnames = {'ShortNameSuffix' 'MapToRelevanceWeights' 'MapToBinaryRelevance' 'CutOffs' 'NotAssessed' 'FixNumberRetrievedDocuments' 'FixedNumberRetrievedDocumentsPaddingStrategy' 'RecallBase' 'Normalize' 'Verbose'};
    dflts =  {[]                []                      []                     []        'NotRelevant' 1000                          'NotRelevant'                                  false        false       false};
    [shortNameSuffix, mapToRelevanceWeights, mapToBinaryRelevance, cutOffs, notAssessed, fixNumberRetrievedDocuments, fixedNumberRetrievedDocumentsPaddingStrategy, recallBase, normalize, verbose, supplied] ...
    ...
    if supplied.RecallBase
        ...
        if supplied.CutOffs
             error('MATTERS:IllegalArgument', 'Cannot compute CG at recall base and CG@DCV at the same time.');
        end; 
    end; 
    ...
    if verbose
        ...
        fprintf('Computing cumultated gain for run set %s with respect to pool %s: %d run(s) and %d topic(s) to be processed.\n\n', ...
    ...
    if ~isempty(cutOffs)
        if numel(cutOffs) > 1
            measuredRunSet.Properties.UserData.name = 'cumulatedGainAtDocumentCutOffValues';        
            measuredRunSet.Properties.UserData.shortName = 'CG_DCV';
        else
            if isnan(cutOffs)
                measuredRunSet.Properties.UserData.name = 'cumulatedGainAtRelevantRetrievedDocuments';        
                measuredRunSet.Properties.UserData.shortName = 'CG_RR';
            else
                measuredRunSet.Properties.UserData.name = ['cumulatedGainAt_' num2str(cutOffs) '_DocumentCutOffValue'];        
                measuredRunSet.Properties.UserData.shortName = ['CG_' num2str(cutOffs)];
            end;
        end;
                
        measuredRunSet.Properties.UserData.cutOffs = cutOffs;
    elseif recallBase
        measuredRunSet.Properties.UserData.name = 'cumulatedGainAtRecallBase';
        measuredRunSet.Properties.UserData.shortName = 'CG_RB';
    else
        measuredRunSet.Properties.UserData.name = 'cumulatedGain';
        measuredRunSet.Properties.UserData.shortName = 'CG';
    end;
    ...
    if verbose
        fprintf('Computation of cumulated gain completed.\n');
    end;
        ...
        function [measure] = processRun(runTopic)
            ...
            if(runSetStats{ct, cr}.relevantRetrieved == 0)                
                ...
                if normalize 
                    ...
                    measure = measure ./ cumsum(sort(idealRun{ct, 1}{1, 1}{:, :}.', 2, 'descend'));
                end;
