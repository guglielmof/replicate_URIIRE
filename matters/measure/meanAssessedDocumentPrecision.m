...
%%
function [measuredRunSet, poolStats, runSetStats, inputParams] = meanAssessedDocumentPrecision(pool, runSet, varargin)
    ...
    if verbose
        ...
        fprintf('Computing mean assessed document precision %s for run set %s with respect to pool %s: %d run(s) and %d topic(s) to be processed.\n\n', ...
    ...
    measuredRunSet.Properties.UserData.name = 'meanAssessedDocumentPrecision';
    measuredRunSet.Properties.UserData.shortName = 'MADP';
    
    if strcmpi(notAssessed, 'condensed')
      measuredRunSet.Properties.UserData.name = 'condensedMeanAssessedDocumentPrecision';
      measuredRunSet.Properties.UserData.shortName = 'condMADP';
    end;
    ...
    if verbose
        fprintf('Computation of mean assessed document precision completed.\n');
    end;
    ...
    function [varargout] = processTopic(topic)
        ...
        function [measure] = processRun(runTopic)
            ...
            % compute precision at retrieved document.
            prec = cumsum(runTopic{:, 'Assessment'}).' ...
                            ./ ...
                          (1:height(runTopic));
                      
            % compute precision at relevant retrieved document and set 0
            % elsewhere
            prec_RR = prec;
            prec_RR(~logical(runTopic{:, 'Assessment'})) = 0;
            
            % create the transition matrix
            P = diag(ones(1, runSetStats{ct, cr}.retrieved - 1)*p, 1) + ...
                diag(ones(1, runSetStats{ct, cr}.retrieved - 1)*q, -1);
            
            num = (eye(runSetStats{ct, cr}.retrieved) - P) \ prec_RR.';
            ...
            den = (eye(runSetStats{ct, cr}.retrieved) - P) \ prec.';
