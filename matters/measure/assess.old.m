...
function [assessedRunSet, poolStats, runSetStats, inputParams] = assess(pool, runSet, varargin)
    ...
    function [varargout] = assessTopic(topic)
        ...
        function [judgement] = assessRun(runTopic)
            ...
            % the rows in runTopic
            [~, loc] = ismember(runTopic{:, 'Document'}, judgement{:, 'Document'});
            
            % remove possible 0 values due to condensed lists (inner-join)
            % which discard (most of) retrieved documents
            loc = loc(loc>0);
            
            judgement = judgement(loc, :);
            
            % remove the useless document column
            judgement.Document = [];
