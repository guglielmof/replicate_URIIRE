%% expectedGradedAveragePrecision
% 
% Computes expected graded average precision (eGAP).

%% Synopsis
%
%   [measuredRunSet, poolStats, runSetStats, inputParams] = expectedGradedAveragePrecision(pool, runSet, Name, Value)
%  
% Note that graded expected graded average precision will be NaN when there are no relevant
...
% |expectedGradedAveragePrecision|; _|shortName|_ is a short name of the computed 
% measure, i.e. |eGAP|; _|pool|_ is the identifier of the pool with respect 
...
% name are, respectively, |condensedExpectedGradedAveragePrecision| and |conEGAP|.
...
%% Example of use
%  
%   measuredRunSet = expectedGradedAveragePrecision(pool, runSet);
%
...
%%
function [measuredRunSet, poolStats, runSetStats, inputParams] = expectedGradedAveragePrecision(pool, runSet, varargin)
    ...
    measuredRunSet.Properties.UserData.name = 'expectedGradedAveragePrecision';
    measuredRunSet.Properties.UserData.shortName = 'eGAP';
    
    if strcmpi(notAssessed, 'condensed')
      measuredRunSet.Properties.UserData.name = 'condensedExpectedGradedAveragePrecision';
      measuredRunSet.Properties.UserData.shortName = 'condEGAP';
    end;
    ...
    if verbose
        fprintf('Computation of expected graded average precision completed.\n');
    end;
    ...
    function [varargout] = processTopic(topic)
        ...
        function [measure] = processRun(runTopic)
            ...
            delta = zeros(K, runSetStats{ct, cr}.relevantRetrieved);
                                    
                    
            for k = 1:runSetStats{ct, cr}.relevantRetrieved
                for j = 1:runTopic{relPos(k), 'Assessment'}
                    delta(j, k) = sum(runTopic{1:relPos(k), 'Assessment'} >= j);
                end;
            end;
            ...
            measure = sum(...
                        ((g ./ (fliplr(cumsum(fliplr(relevanceCount))))) * delta) ...
                            ./ ...
                        relPos ...
                      );
